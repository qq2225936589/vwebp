...
void motion_callback_func(int x, int y)
{
    if (isMove)
    {
        POINT pos;
        GetCursorPos(&pos);
        glutPositionWindow(pos.x - offX, pos.y - offY);
    }
}
...
static void StartDisplay(void) {
...
#if defined(__APPLE__) || defined(_WIN32)
  glutInitDisplayMode(GLUT_RGBA | GLUT_BORDERLESS);
#else
  ...
  glutMotionFunc(motion_callback_func);
